...
export default function DetailPlnPage() {
  const params = useParams();
  const router = useRouter();
  const [activeCategory, setActiveCategory] = useState("PLN");
  const slug = params.slug as string;

  const currentItems = productsData["PLN"]?.items || {};
    ...
    return (
      <main className="min-h-screen bg-[#070B14] text-white overflow-x-hidden">
        ...
        <div className="p-20 text-center text-white">
          ...
          <button onClick={() => router.push("/pln")} className="mt-4 text-[#FACC15] font-bold hover:underline">
            Kembali ke PLN
          </button>
        </div>
        <FaqAndFooter />
  ...
  return (
    <main className="min-h-screen bg-[#070B14] text-white overflow-x-hidden">
      ...
      <div className="pt-4 pb-12">
        <TransactionForm product={mockProduct} onBack={() => router.push("/pln")} />
      </div>
      <FaqAndFooter />
    </main>
  );
